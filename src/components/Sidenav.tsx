    ...
    >
      ...
      <div className="flex gap-4 items-center">
        <img
          ...
          className={`cursor-pointer duration-500 w-16 rounded-lg ${isOpen && "rotate-[360deg]"
            } `}
      ...
      <ul className="pt-6">
        {sideNavMenu.map((item, index) => (
          <a
            key={index}
            className={`text-primary text-sm font-medium flex gap-4 mt-2 items-center cursor-pointer p-2 hover:bg-secondary-100 hover:rounded-md ${currentPath === item.path ? "bg-secondary-100 rounded-md" : ""
              }`}
            href={item.path}
          >
            {item.icon}
        ...
        <div className="w-full flex justify-center items-center p-2">
          <PrimaryButton
            style={`text-xs absolute bottom-5  flex gap-2 justify-center items-center ${isOpen ? "w-4/5" : "w-3/5"
              }`}
            label={`${isOpen ? "Logout" : ""}`}
